...
def create_svg(width, height, wall_map, ducts, blue, green, purple):
    dwg = svgwrite.Drawing(SVG_OUTPUT_PATH, size=(width, height))

    mask_to_svg(dwg, wall_map, "rgb(120,120,128)")
...
def home():
    ...
    <html>
    <head>
        <title>BAS Graphic Generator v3</title>
    </head>
    <body style="margin:0;background:#101218;color:white;font-family:Arial, Helvetica, sans-serif;">
        <div style="max-width:950px;margin:55px auto;background:#1b1d25;padding:42px;border-radius:24px;text-align:center;border:1px solid #303542;">
            ...
            <h1>BAS Graphic Generator v3</h1>
            <p style="color:#b8bcc8;">Wall Reconstruction · AI Detection · Isometric Engine</p>
...
def generate():
    ...
    wall_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (9, 9))
    ...
    room_fill = cv2.GaussianBlur(wall_map, (31, 31), 0)
    room_fill = cv2.threshold(room_fill, 12, 255, cv2.THRESH_BINARY)[1]

    # =========================
    # COLOR-GUIDED BAS DETECTION
    # =========================
    ...
    canvas[room_fill > 0] = (35, 38, 48, 70)
    canvas[wall_map > 0] = (135, 135, 145, 255)
    ...
    create_svg(target_w, target_h, wall_map, ducts, blue, green, purple)
    ...
    <body>
        ...
        <p style="text-align:center;color:#b8bcc8;">
            Wall Reconstruction v1 · Room Fill · Isometric Engine
        </p>
